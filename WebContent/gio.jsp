...
<body>
 ...
 <table class="table">
    	...
    	<c:forEach items="${dsgh }" var="g"> 
    		<form method="post" action="suagio?mh=${g.getMahang() }"><tr>
    			<td><img src="${g.getAnh() }" width="60" height="80"></td>
    				<td>${g.getTenhang() }
    				<br>${g.getDungtich() }</td>
    				<td><fmt:formatNumber type="number" 
        		maxFractionDigits="3" value="${g.getGia() }" />₫</td>
    				<td>x ${g.getSlmua() }</td><td>
    				<input name="txtsl" value="${g.getSlmua() }" type="number">
					 <br>
		...
		<form method="post" action="sanpham">
		  		<input name="but" type="submit" value="< Tiếp tục mua hàng" class="btn-primary">
		 </form>
		 </th>
		 <th colspan="2"> 
		<form method="post" action="thanhtoan">
		  		<input name="but" type="submit" value="Thanh toán >" class="btn-primary">
		 </form> </th>
		 </tr>
    	</c:when><c:otherwise><tr><td><h4> Giỏ hàng trống ! Hãy thêm giỏ hàng </h4></td></tr>
