...
<body class="g-sidenav-show  bg-gray-200">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <c:choose>
				...
				<c:when test="${sessionScope.dn.getQuyen()==false}">
				
						<h3>Tài khoản không đủ quyền để quản lý mục này !</h3>
          ...
          <div class="card my-4">
            ...
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">MẶT HÀNG</h6>
            ...
            <div class="card-body px-0 pb-2">
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  <thead>
                  <form method="post" action="quanlyhang?them">
       		<input name="but1" type="submit" value="Thêm hàng" class="btn btn-success">
       </form>
                    <tr>
                      ...
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">${h.getMahang()}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0">${h.getTenhang()}</p>
                         <p class="text-xs text-secondary mb-0">${h.getDungtich()}</p>
                         <p class="text-xs text-secondary mb-0">Giá :<fmt:formatNumber type="number" 
        		maxFractionDigits="3" value="${h.getGia() }" />₫</p>
        				<p class="text-xs text-secondary mb-0">Mã loại : ${h.getMaloai()}</p>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0">${h.getSoluong()}</p>
                      </td>
                      <td>
                        <img src="${h.getAnh()}" style="width: 80px;height: 40px">
                      </td>
                      <td> 
            	<form action="xulihang" method="post">
            		 <input id="msxoa" type="hidden" name="msxoa" value="${h.getMahang()}">
            		<button type="submit" class="btn btn-danger">Xoá</button>
            	</form></td>
            	    <td> <a href="quanlyhang?mahang=${h.getMahang()}&tenhang=${h.getTenhang()}&soluong=${h.getSoluong()}&gia=${h.getGia()}&anh=${h.getAnh()}&maloai=${h.getMaloai()}&mota=${h.getMota()}&thanhphan=${h.getThanhphan()}&hdsd=${h.getHdsd()}&congdung=${h.getCongdung()}&dungtich=${h.getDungtich()}" class="btn btn-success">Cập nhật</a></td>
